...
#### 7.22(화)
```
피어 피드백 시간에 했던 말들이 기억 안 나는 경우가 많아서 3번 미션(피어 피드백 녹음 뒤 자동 요약)을 선택했습니다.
화,수,목,금 총 4일 수행할 예정이었습니다.

그러나..
ZOOM, 클로바노트 녹음을 동시에 켜니 노트북 RAM이 견디지 못하고
ZOOM이 멈추고 꺼져버렸습니다.ㅠㅠ
```
...
---
### J092
클로바 노트 미션은 다음과 같은 방식으로 진행했다.
1. 음성 녹음으로 녹음하기 (클로바 노트에서 녹음한 적이 있었는데, 잘못 누르고 삭제하여 다시 볼 수 없었다... 그래서 음성 녹음에서 녹음하는 방식으로 바꿨다.)
2. 음성 녹음을 클로바 노트에 업로드하여 AI 요약 확인하기
	- 녹음한 파일을 클로바 노트에 업로드하면 AI가 요약된 내용(주요 주제, 다음 할일, 단락별 요약)을 보여준다.
	- <img width="387" height="382" alt="image" src="https://gist.github.com/user-attachments/assets/48fbbd61-1676-41c2-bbf1-645bf7b6375b" />
...
3. ChatGPT에 요약 내용을 입력하여 중요 개념 정리 요청하기
	...
	- 🎯 중요 개념 한 줄 요약 리스트
    1. **인메모리 파일 시스템 설계**  
        → 디스크 없이 메모리 구조만으로 파일 시스템의 작동 원리를 구현할 수 있다.
    2. **싱글톤 패턴**  
        → 전역 상태 공유와 제어가 필요한 시스템에서 하나의 인스턴스를 보장하는 설계 방식.
    3. **파일 시스템의 기본 개념**  
        → 파일과 디렉토리를 트리 구조로 관리하며, 메타데이터와 경로 기반으로 접근된다.
    4. **FCB(File Control Block)**  
        → 파일의 속성과 위치 정보를 저장하는 구조로, OS가 파일을 추적·관리하는 핵심 단위.
#### 후기
